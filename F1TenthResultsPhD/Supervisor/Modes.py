    ...
    def plot_mode_picture(self):
        ...
        for i in [12]:
            ...
            for j in range(self.n_modes):
                n_mode = self.mode_table[i, j]
                if j == i:
                    plt.plot(self.qs[j][0], self.qs[j][1], color='#3498DB', marker='X', markersize=14)
                    plotted.append(n_mode)
                elif n_mode != -1:
                    plt.plot(self.qs[n_mode][0], self.qs[n_mode][1], color='#58D68D', marker='o', markersize=8)
                    plotted.append(n_mode)

            for j, m in enumerate(self.qs):
                if not j in plotted:
                    plt.plot(m[0], m[1], 'o', color="#E74C3C", markersize=8)
...
if __name__ == "__main__":
    kernel_conf = load_conf("kernel_config")
