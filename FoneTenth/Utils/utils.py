...
def save_kernel_config(conf_namespace: Namespace, save_name, directory="Data/Kernels/"):
    dictionary = conf_namespace.__dict__
    path = directory + f"/{save_name}.yaml"
    with open(path, 'w') as file:
        yaml.dump(dictionary, file)

def load_kernel_config(save_name):
    path = f"{save_name}.yaml"
    with open(path) as file:
        kernel_conf = yaml.load(file, Loader=yaml.FullLoader)
    
    kernel_conf = Namespace(**kernel_conf)
    ...
    for rep in range(run_dict['n']):
        for run in run_dict['runs']:
            ...
            if run['planner'] == "Agent":
                run['run_name'] = f"{run['racing_mode']}_{run['train_mode']}_{run['test_mode']}_{run['reward']}_{run['map_name']}_{set_n}_{rep}"
            else:
                run['run_name'] = f"{run['racing_mode']}_{run['planner']}_{run['test_mode']}_{run['map_name']}_{set_n}_{rep}"
